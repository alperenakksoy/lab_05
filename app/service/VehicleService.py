...
def get_all(db: Session, skip: int = 0, limit: int = 100) -> list[VehicleOut]:
    ...
    for v in vehicles:
        ...
        object.__setattr__(out, "_links", _links(v.id))  # FIXED: _links artik dolduruluyor
        result.append(out)
    return result
...
def create(db: Session, data: VehicleCreate) -> VehicleOut:
    ...
    out = VehicleOut.model_validate(v)
    object.__setattr__(out, "_links", _links(v.id))
    return out
...
def get_all_v2(db: Session, skip: int = 0, limit: int = 100) -> list[VehicleOutV2]:
    ...
    for v in vehicles:
        ...
        object.__setattr__(out, "_links", _links(v.id, "v2"))
        result.append(out)
    return result
